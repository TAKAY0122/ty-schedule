...
async function api(req, env, url) {
  ...
  // ---- チャット(ポーリング方式) ----
  // 役職別チャット(type='notice')のref_key→閲覧に必要な最低ロールレベル。LVの階層どおり
  // 上位ロールほど累積で閲覧できる(例: 'chief'部屋はchief/handler/adminが見られる)。通常の
  // 会話が可能なチャットルームで、'all'(全員)向けの内容は既存の全体チャット(type='all')に
  // 投稿するため、ここには含めない(2026年9月、ユーザーの明示的な指示。「お知らせ専用」の
  // 別ルームにはしない)。アップデートのお知らせは、対象ロールに応じてここ、または全体チャットへ
  // sender_name「お知らせ」のシステムメッセージとして自動投稿される。
  const NOTICE_ROOM_TIERS = ['chief', 'handler', 'admin'];
  const NOTICE_ROOM_MIN_LV = { chief: 1, handler: 2, admin: 3 };
