...
const UPDATE_ITEMS = [
  ...
  { v:12, icon:'moon', title:'ダークモードに対応', desc:'ドロワーメニュー最下部の「ダークモードに切替」から、アプリ全体を暗い配色に切り替えられるようになりました。設定は端末ごとに保存され、次回以降も自動で適用されます。', show: () => ME.role === 'admin' },
  { v:12, icon:'search', title:'現場名・会場を一覧から選べるように', desc:'現場変更の報告・承認や、手配チームのスケジュール入力で、現場名の隣の虫めがねボタンから公演一覧を、会場の隣から会場一覧を検索して選べるようになりました。選ばずに直接入力する使い方もこれまで通りできます。', show: () => ME.role === 'admin' },
  { v:12, icon:'key', title:'モーダルをキーボードだけで操作できるように', desc:'入力・確認用のポップアップ画面で、Enterキーで決定・Escapeキーで閉じる・Tabキーで画面内の項目だけを巡回する、といった操作がキーボードだけでできるようになりました(Windows/Mac共通)。', show: () => ME.role === 'admin' },
